...
>(({ className, value, onChange, iconClass, iconSize = 16, ...props }, ref) => {
  ...
  return (
    <Pressable
      ...
      className={cn(
        'web:peer flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-primary bg-card web:ring-offset-background web:disabled:cursor-not-allowed web:disabled:opacity-50',
        className
      )}
      onPress={() => {
        onChange(!value);
