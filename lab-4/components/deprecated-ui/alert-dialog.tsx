...
const AlertDialogContext = React.createContext<IAlertDialogContext>({} as IAlertDialogContext);
...
>(({ className, children, animationType = 'fade', overlayClass, ...props }, ref) => {
  ...
  return (
    <Modal
      ...
      {...props}>
      <Pressable
        onPressOut={closeOnOverlayPress ? () => setVisible(false) : undefined}
        className={cn(
        ...
        <Pressable
          className={cn(
            'rounded-2xl border border-border bg-background p-8 shadow-lg shadow-foreground/5',
            className
          )}
          role="alertdialog">
          {children}
