...
interface ICommandContext<T extends Data> {
  data: T[];
  onItemSelected: (item: Exclude<T, string>) => void;
  isOpen: boolean;
  toggleIsOpen: () => void;
...
const CommandContext = React.createContext<ICommandContext<any>>({} as ICommandContext<any>);
...
function useCommandContext<T extends Data>() {
  const context = React.useContext<ICommandContext<T>>(CommandContext);
  if (!context) {
...
function CommandTextInput<T extends Data>(
  ...
  return (
    <Animated.View
      ...
      <Button variant="ghost" className="pl-3 pr-0" size="sm" onPress={toggleIsOpen}>
...
>(({ className, textClass, children, ...props }, ref) => {
  return (
    <Pressable
      ref={ref}
      className={cn('border-t border-border bg-background px-5 pt-6', className)}
      {...props}>
      {isTextChildren(children) ? (
        <Text className={cn('px-0.5 text-base font-semibold text-muted-foreground', textClass)}>
          {children}
        </Text>
      ) : (
        children
