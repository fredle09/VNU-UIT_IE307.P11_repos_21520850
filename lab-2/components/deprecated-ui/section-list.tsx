    ...
    (info: Parameters<NonNullable<ShopifyFlashListProps<TItem | string>['renderItem']>>[0]) => {
      if (typeof info.item === 'string' && renderSectionHeader) {
        ...
        return renderSectionHeader?.(
          info as Parameters<NonNullable<ShopifyFlashListProps<string>['renderItem']>>[0]
        );
      } else {
        // Render item
        return (
          renderItem?.(
            info as Parameters<NonNullable<ShopifyFlashListProps<TItem>['renderItem']>>[0]
          ) ?? null
        );
      }
    },
 ...
 */
function useStickyHeaderIndices<TItem>(data: (string | TItem)[], extraData?: unknown) {
  const [stickyHeadersUpdate, triggerStickyHeadersUpdate] = React.useState<boolean>(false);
